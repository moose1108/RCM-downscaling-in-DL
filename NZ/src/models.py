...
def train_model(model, training_data, x_val, y_val, loss, model_weights_name, epochs, batch_size, optimizer, metrics=['mse']):
    ...
    # print(type(training_data), type(training_data[0]), type(training_data[1]), type(x_val), type(y_val))
    # a = input()
...
def my_conv(layer_filters=[16, 64, 128], bn=True, padding='same', kernel_size=5,
                pooling=True, dense_layers=[256], dense_activation='relu', input_shape=(141, 161, 7), terrain_features=None,
    ...
    x = contruct_base_conv(inputs1, layer_filters=layer_filters, bn=bn, padding=padding,
        kernel_size=kernel_size, pooling=pooling, activation=activation, dropout=dropout, terrain_features=terrain_features)

    if terrain_features is not None:
        terrain_features = tf.expand_dims(terrain_features, axis=-1)
        batch_size = tf.shape(inputs1)[0]  # Dynamic batch size extraction
        ...
        terrain_features = tf.expand_dims(terrain_features, axis=0)  # Add batch dimension
        terrain_features = tf.tile(terrain_features, [batch_size, 1, 1, 1])
        concatenated_tensor = tf.concat([inputs1, terrain_features], axis=-1)

        x = contruct_base_conv(concatenated_tensor, layer_filters=layer_filters, bn=bn, padding=padding,
                kernel_size=kernel_size, pooling=pooling, activation=activation, dropout=dropout, terrain_features=terrain_features)
...
def contruct_base_conv(x, layer_filters=[50, 32, 16], bn=True, padding='same', kernel_size=3, pooling=True,
                        activation='relu', dropout=0.5, strides=1, terrain_features=None):
    """
