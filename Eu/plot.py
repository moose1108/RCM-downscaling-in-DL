...
def compute_corr(pred, gt):
    valid_mask = ~np.isnan(pred) & ~np.isnan(gt)
    if np.any(valid_mask):
        return np.corrcoef(pred[valid_mask], gt[valid_mask])[0, 1]
    return np.nan

def compute_mae(pred, gt):
    valid_mask = ~np.isnan(pred) & ~np.isnan(gt)
    if np.any(valid_mask):
        return np.mean(np.abs(pred[valid_mask] - gt[valid_mask]))
    return np.nan

def compute_rmse(pred, gt):
    valid_mask = ~np.isnan(pred) & ~np.isnan(gt)
    if np.any(valid_mask):
        return np.sqrt(np.mean((pred[valid_mask] - gt[valid_mask])**2))
    return np.nan
...
selected_days = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 361] # [i for i in range(180, 210)]# [135] # 
...
for i, (day, date) in enumerate(zip(selected_days, dates)):
    ...
    daily_gt = gt_rain_masked[day, :, :] * factor
    valid_mask = ~np.isnan(daily_pred) & ~np.isnan(daily_gt)
...
corr_list, mae_list, rmse_list = [], [], []
for i, date in enumerate(date_range):
    if date.month == 2 and date.day == 29:  # Skip leap day for consistency
        continue
    
    daily_pred = rain[i, :, :] * factor
    daily_gt = gt_rain[i, :, :] * factor
    
    corr = compute_corr(daily_pred, daily_gt)
    mae = compute_mae(daily_pred, daily_gt)
    rmse = compute_rmse(daily_pred, daily_gt)
    
    corr_list.append(corr)
    mae_list.append(mae)
    rmse_list.append(rmse)
    
    print(f'{date}: Corr={corr:.3f}, MAE={mae:.3f}, RMSE={rmse:.3f}')

yearly_corr = np.nanmean(corr_list)
yearly_mae = np.nanmean(mae_list)
yearly_rmse = np.nanmean(rmse_list)

print("\n===== Yearly Metrics =====")
print(f'Yearly Correlation: {yearly_corr:.3f}')
print(f'Yearly MAE: {yearly_mae:.3f}')
print(f'Yearly RMSE: {yearly_rmse:.3f}')
print("Prediction Min/Max:", np.nanmin(rain * factor), np.nanmax(rain * factor))
print("Ground Truth Min/Max:", np.nanmin(gt_rain * factor), np.nanmax(gt_rain * factor))
